...
int main(int argc, char **argv) {
    ...
    for (int i = nrl + 1; i < nrh; i++) {
        for (int j = ncl + 1; j < nch; j++) {
            ...
            output_img_edges_detection[i][j] = gradient_norm > EDGE_THRESHOLD ? 255 : 0;
        }
    }
    ...
    SavePGM_bmatrix(output_img_edges_detection, nrl, nrh, ncl, nch, "data/output/cubes_edges_detection_t" STR(EDGE_THRESHOLD) ".pgm");
